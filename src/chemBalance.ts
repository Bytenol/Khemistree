 ...
 * @returns {*[]}
 */
const extractAtom = (txt: string) => {
    let atoms: vector_t<iAtom> = [];
 ...
 */
const groupReactantProduct = (text: string) => {
 ...
 */
const makeAtomNodeLst = (formula: string) => {
    let currentNode: AtomNode | null = null;
    let stack: vector_t<AtomNode> = [];
    let res: vector_t<AtomNode | undefined> = [];
...
const initEqnCoeff = (paramLst: any) => {
    let paramEqn: any[] = [];
    let paramDict = { };
...
const initEqnMatrix = (lhs: { eqn: any; set: any; }, rhs: { eqn: any; set?: Set<unknown>; }) => {
    ...
    for(let i = 0; i < atoms.length; i++) {
        ...
        for(let j = 0; j < allEqn.length; j++) {
            const eqn = allEqn[j];
            const atomFound = eqn.filter((i: { symb: any; }) => i.symb === atom);
...
const BalanceMolecularEqn = (text: string) => {
    error = "";
    const [reactants, products] = groupReactantProduct(text) || [[""], [""]];
    const res: { reactants?: string[], products?: string[], error?: string, coeff?: number[] } = { reactants, products, error: "" };
    ...
    while(true) {
        gcdIndex++;
        if(g === 1 || gcdIndex === nums.length - 2) break;
        g = gcd(g, gcdIndex + 1);
    }

    if(Math.abs(g) !== 1) nums = nums.map(i => {
        i /= g;
        return i;
    });
